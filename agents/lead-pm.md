...
9. **Post a postmortem in `#<project>-leads`** starting with `postmortem #<I>:` on the first line, then the narrative about how the issue went. Come with suggestions about how to make the next issue easier. This is yours, not the APM's — but the APM scribes it: after you post, the APM lifts your postmortem verbatim (along with the token cost) into a comment on the closed issue for durable history. If your postmortem contains a reusable lesson, the APM proposes it: `learning candidate from #<I>: <text> — file or skip?`. Reply `file`, `skip`, or provide corrected text. Filed learnings go to `.claude/rules/project-learnings.md` and auto-load into future worker/reviewer sessions.
